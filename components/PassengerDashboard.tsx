...
const DocumentIcon = ({ type }) => {
  switch (type) {
    case 'cartao_embarque': return html`<div className="p-3 bg-[#EE8F66]/10 text-[#EE8F66] rounded-xl"><${ArrowRight} size=${20} className="-rotate-45" /></div>`;
    case 'e-ticket': return html`<div className="p-3 bg-[#EE8F66]/10 text-[#EE8F66] rounded-xl"><${FileText} size=${20} /></div>`;
    case 'voucher_hospedagem': return html`<div className="p-3 bg-[#A39161]/10 text-[#A39161] rounded-xl"><${Hotel} size=${20} /></div>`;
    default: return html`<div className="p-3 bg-slate-100 text-slate-500 rounded-xl"><${FileText} size=${20} /></div>`;
  }
};

const AgendaItemIcon = ({ type }) => {
  switch (type) {
    case 'flight': return html`<div className="text-[#EE8F66]"><${Plane} size=${18} /></div>`;
    case 'hotel': return html`<div className="text-[#A39161]"><${Hotel} size=${18} /></div>`;
    case 'activity': return html`<div className="text-[#EE8F66]"><${Star} size=${18} /></div>`;
    default: return html`<div className="text-slate-400"><${Clock} size=${18} /></div>`;
  }
};
    ...
    <div className=${`relative p-6 rounded-[2.5rem] border transition-all duration-300 ${suggestion.isPurchased ? 'bg-green-50/30 border-green-200' : isAgency ? 'bg-white border-[#A39161]/20 shadow-lg shadow-[#A39161]/5' : 'bg-white border-[#EE8F66]/10 hover:border-[#EE8F66]/30 hover:shadow-xl'}`}>
      <div className="flex justify-between items-start mb-4">
        <div className=${`p-3 rounded-2xl ${suggestion.isPurchased ? 'bg-green-100 text-green-600' : isAgency ? 'bg-[#A39161]/10 text-[#A39161]' : 'bg-[#FFFAF5] text-[#EE8F66]'}`}>
          ${suggestion.isPurchased ? html`<${CheckCircle2} size=${20} />` : html`<${Compass} size=${20} />`}
        </div>
        ${suggestion.isPurchased ? html`
...
const PassengerDashboard = ({ trips, onOpenChat, onLogout, hideHeaderFooter = false }) => {
  ...
  return html`
    <div className="space-y-12 animate-in fade-in duration-1000 pb-32">
      ${hideHeaderFooter && onLogout && html`
        <button 
          onClick=${onLogout}
            ...
            ${activeTrips.map(trip => html`
              <div key=${trip.id} className="space-y-12">
                ...
                <div className="space-y-8">
                  ...
                  ${loadingSuggestions[trip.id] && !trip.recommendations?.length ? html`
                  ...
                  ` : html`
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ${getAllSuggestions(trip).map((s, i) => html`<${SuggestionCard} key=${i} suggestion=${s} />`)}
                    </div>
                  `}
                </div>
              </div>
